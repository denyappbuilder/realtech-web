...
test("karta tématu má právě jeden odkaz a ten vede na /temata/{slug}/", () => {
  ...
  assert.match(karta, /<h2><a href=\{`\/temata\/\$\{t\.slug\}\/`\}>/, "titulek karty neodkazuje na detail tématu");
});
...
test("odkaz v titulku je roztažený přes celou kartu (stretched link)", () => {
  ...
  const stretched = styl.match(/\.card-body h2 a::after,\s*\.card-body h3 a::after \{([\s\S]*?)\}/)?.[1] ?? "";
...
test("karta tématu bere <picture> z nahledKarty(nejnovejsi), ne prázdné card-body", () => {
  ...
  assert.doesNotMatch(karta, /<a href=\{`\/temata\/\$\{t\.slug\}\/`\}>(?:(?!<\/a>)[\s\S])*<picture>/, "náhled nesmí být uvnitř odkazu; uzavřený titulek může předcházet fotce");
