...
//
// src/components/SearchModal.astro (139 řádků) nespouštěl dosud ŽÁDNÝ test —
...
// protože obě strany si tentýž text vykládají jinak — viz WEB-SEARCH-002.
...
// ---------------------------------------------------------------------------
// Nálezy — chování, které je dnes špatně. Opravu sem NEmíchám.
// ---------------------------------------------------------------------------
...
test.todo(
  ...
  () => {
    ...
    modal.render([polozka({ s: 'a', t: 'Sonda <img src=x> a Ostrava & okolí', k: '<b>AI</b>' })], 'q');

    // Titulky i kategorie jdou do innerHTML bez escapování — libovolné `<`
    // ve frontmatteru rozbije seznam výsledků (dnes tam žádné není, ale
    // schéma `title` je volný z.string(), takže tomu nic nebrání).
    assert.doesNotMatch(modal.results.innerHTML, /<img src=x>/);
    assert.doesNotMatch(modal.results.innerHTML, /<b>AI<\/b>/);
    assert.match(modal.results.innerHTML, /Ostrava &amp; okolí/);
  },
);
