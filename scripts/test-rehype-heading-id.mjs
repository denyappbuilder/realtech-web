...
assert.equal(plugins.length, 1, 'astro.config.mjs má mít právě jeden rehype plugin');

/** Vyrobí transformer — plugin je factory, volá se jednou za dokument. */
const transform = () => plugins[0]();
...
test(
  'dva nadpisy se stejným textem musí dostat různá id [codex-testy-web/REHYPE-NADPISY-001]',
  { todo: 'REHYPE-NADPISY-001 — plugin přepíše deduplikovaný slug od Astra vlastním, bez pořadového čísla' },
  () => {
    const prvni = el('h2', [text('Shrnutí')]);
    const druhy = el('h2', [text('Shrnutí')]);
    ...
    assert.notEqual(
      idOf(druhy),
      idOf(prvni),
      'duplicitní id v HTML — kotva na druhou sekci skočí na první',
    );
  },
);

test(
  'nadpisy shodné v prvních 60 znacích musí dostat různá id [codex-testy-web/REHYPE-NADPISY-001]',
  { todo: 'REHYPE-NADPISY-001 — kolizi vyrobí i samotný ořez na 60 znaků' },
  () => {
