...
function runGenerator(root) {
  return execFileSync(
    process.execPath,
    ['--no-warnings', '--import', register, generator],
...
test('standardní frontmatter vytvoří OG bez skutečného obrazového renderu', (t) => {
  ...
  assert.equal(fs.readFileSync(generatedImage(root), 'utf8'), 'sharp mock output\n');
});

test('`---` v quoted YAML hodnotě před image nesmí zabránit vytvoření OG', (t) => {
  const root = createFixture(t, [
