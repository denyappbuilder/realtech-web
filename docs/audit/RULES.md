...
- Úprava existujícího testu (Daniel, 19. 9. 2026): existující test smí změnit jen položka, která záměrně mění přesně to chování, které test hlídá. Postup: (1) v git historii zjistit, proč test vznikl — pokud commit zaznamenává výslovné rozhodnutí člověka (např. „Maky nechce…“), je to produktové rozhodnutí a položka se parkuje; (2) jinak upravit jen asserty svázané s měněným chováním — žádný test nemazat, nevypínat, neoslabovat nesouvisející kontroly; (3) pro nové chování přidat vlastní testy; (4) v PR samostatná sekce „Změny testů“: test, starý assert, nový assert, důvod.
